...
# check root
[[ $EUID -ne 0 ]] && LOGE "${red}error: ${plain} This script must be run as root!\n" && exit 1
...
if [[ -f /etc/redhat-release ]]; then
    ...
    LOGE "${red}System version not detected, please contact the script author!${plain}\n" && exit 1
fi
...
update() {
    confirm "This function will force reinstall the current latest version，Data will not be lost, continue or not?" "n"
    if [[ $? != 0 ]]; then
        LOGE "Cancelled"
        if [[ $# == 0 ]]; then
            before_show_menu
...
reset_user() {
    confirm "Are you sure you want to reset the username and password to admin 吗" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
ssl_cert_issue() {
    ...
    LOGI "4.The default installation path for this script to apply for a certificate is /root/cert.crt"
...
show_usage() {
    ...
    echo "x-ui enable       - setup x-ui Auto-start"
...
————————————————
  ${green}13.${plain} enable x-ui Auto-start
  ${green}14.${plain} disable x-ui Auto-start
————————————————
  ${green}15.${plain} key installation bbr (latest kernel)
